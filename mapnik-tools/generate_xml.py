...
def serialize(xml, options):
    try:
        ...
        try:
            ...
            sys.exit(color_text(
                1, 'Error: saving xml requires Mapnik python bindings to be installed'))
    m = mapnik.Map(1, 1)
    ...
    if options.output:
        ...
        if hasattr(mapnik, 'mapnik_version') and mapnik.mapnik_version() >= 700:
            ...
            sys.exit(color_text(
                1, 'Minor error: printing XML to stdout requires Mapnik >=0.7.0, please provide a second argument to save the output to a file'))


def validate(params, parser):
    if not os.path.exists(params['world_boundaries']):
        parser.error("Directory '%s' used for param '%s' not found" %
                     (params['world_boundaries'], 'world_boundaries'))
    supported_srs = [900913, 4326]
    if not int(params['epsg']) in supported_srs:
        parser.error('Sorry only supported projections are: %s' %
                     supported_srs)
...
if __name__ == '__main__':
    ...
    if '--inc' in sys.argv:
        ...
        if not len(sys.argv) > (idx + 1) or '--' in sys.argv[idx + 1]:
            parser.error(
                "--inc argument requires a path to a directory as an argument")
        else:
    ...
    parser.add_option('--inc', dest='inc',
                      help="Includes dir (default: '%s')" % inc_dir)

    parser.add_option('--accept-none', dest='accept_none',
                      action='store_true', help="Interpret lacking value as unneeded")
    ...
    if not includes:
        parser.error(
            "Can't find include templates, please provide search path using '--inc' , currently using '%s'" % search_path)
    ...
    if not matches:
        parser.error(color_text(
            1, "Can't properly parse out variables in include templates.\nMake sure they are all wrapped like '%(variable)s'"))
    ...
    if p.missing:
        parser.error(color_text(1, "\nPlease provide the following parameters values (or set as env variables):\n%s" % ''.join(
            [" --%s 'value' " % v for v in p.missing]) + help_text))
